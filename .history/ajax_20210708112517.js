    ...
    $.ajax({
        ...
        success:function(data){
            ...
            for(i=0; i<x.length; i++){
                output += "<tr><td>" + x[i].id + "</td><td>" + x[i].id + "</td><td>" + x[i].name + " </td><td> " + x[i].number + " </td><td> <button></button> </td><td> <button class='btn btn-danger btn-sm btn-delete' data-sid=" + x[i].id+ " >delete</button></td></tr> " 
            }
            $("#tbody").html(output);
